...
exports.amendment = (place, opts) => {
    ...
    Hoek.assert(internals.amendments[place], `Manifest does not have a default entry for "${place}".`);
...
exports.amendments = (overrides) => {
    ...
    const { include, exclude } = defaults;
    defaults.exclude = (include || exclude) ? exclude :
        (_, path) => {

            const parts = path.split(Path.sep);
            const [filename] = parts.slice(-1);

            return parts.includes('helpers') ||
                Path.basename(filename, Path.extname(filename)) === 'helpers';
        };
    ...
    plugins: {
        ...
        useFilename: (value, filename) => {
            ...
            if (!value.plugins) {
                ...
                value = Hoek.clone(value, { shallow: true });
                value.plugins = Hoek.clone(value.plugins, { shallow: true });
